...
Intuitively, it is totally fine for a \textit{convincing-Johnny-to-ride-this-horse} event to be judged as \textit{tough}. The unacceptability of (\ref{ex:emb-pc})  seems to come from the fact that a \textit{convincing-Lucy-to-look-at-this-painting} event does not constitute a suitable circumstance for a prettiness judgment about the painting. In other words, a \textit{convincing}-event is not susceptible to \textit{cause} a prettiness judgment. This also suggests that the impossibility of long-distance dependencies in \textit{pretty}-constructions is not syntactic, but rather, \textit{appears syntactic} due to embedding predicates (raising, control, attitude...) being generally incompatible with direct sensory experience. 
 	...
 	\iffalse
 		\item To flesh out this intuition, let us consider the two following scenarios.
 		\begin{tcolorbox}
 			\begin{itemize}
 				\item \underline{\textbf{Background for both Scenarios}:} \textit{Tomorrow in San Diego will be the start of the biggest horse race that has ever happened in America. Johnny is a former jockey and is looking forward to attending the kick-off event.}
 				\item \underline{\textbf{Scenario 4}:} \textit{Johnny is aware that he may oversleep and show up late at the event; in that eventuality, he would end up very far from the starting line, surrounded by people that are way taller than him, and would hardly see anything}.
 				\item \underline{\textbf{Scenario 5}:} \textit{Johnny is convinced that he will show up early at the event, and that he will manage to get the best seats, closest to the starting line. For sure, all the conditions will be met for him to enjoy the race.}
 			\end{itemize}
 		\end{tcolorbox}
 		\item The following sentence:
 		\begin{exe}
 			\ex[] {Johnny: \textbf{The race} is pretty to look at.}
 		\end{exe}
 		Appears clearly felicitous under \textbf{Scenario 5} and clearly infelicitous under \textbf{Scenario 4}, even tough in both scenarios the race had the same inherent qualities, and Johnny, the same standards.
 		\item This again means that \textit{pretty}-predicates are sensitive to some properties of the circumstances in which the \textit{prettiness} judgment is expressed by the judge.
 	\fi
This causality relationship between the \textit{pretty} predicate and the infinitival clause in turn suggests that the infinitival clause constitutes the \textsc{Reference} argument of \textit{pretty}. To flesh out this intuition, let us try to replicate the effect established for \textit{tough}-subject \textit{via} \textbf{Scenario 1} and \textbf{Scenario 2} back in Section \ref{ex:fronting-strategies-tough}. In the case of the \textit{pretty}-construction, we will need to make the content embedded clause vary, and see if any interpretive differences arise. This as we will see, requires some creativity in the formulation of the Scenarios, because \textit{pretty}-predicates, due to their sensory component, only allow for a reduced number of embedded events in the general case. We tried to circumvent this restriction in the following two Scenarios:

\begin{center}
	\fbox{
		\begin{minipage}{.8\linewidth}
			\textbf{\textbf{\underline{Scenario 4:}}} \textit{In year 3059, people are often subject to chronic ageusia (loss of taste and smell). Ingenious brain implants have been developed however, that allow to ``wire'' visual perception to a feeling of gustatory pleasure or disgust.}
		\end{minipage}
	}
\end{center}
\begin{center}
	\fbox{
		\begin{minipage}{.8\linewidth}
			\textbf{\textbf{\underline{Scenario 5:}}} \textit{In year 3059, people are often subject to chronic ageusia (loss of taste and smell). Ingenious brain implants have been developed however, that allow to ``wire'' tactile perception at the level of the mouth to a feeling of gustatory pleasure or disgust.}
		\end{minipage}
	}
\end{center}
...
\begin{exe}
	\ex 
	\begin{xlist}
		\ex[] {This cherry pie is delicious to look at.}\label{ex:tasty-visual}
		\ex[] {This cherry pie is delicious to feel.}\label{ex:tasty-tactile}
...
\begin{center}
	\fbox{
		\begin{minipage}{.8\linewidth}
			\centering
			$\llbracket$pretty$\rrbracket$^j = $\lambda C_{\langle \nu \langle s t\rangle \rangle} . \ \lambda x_{e\setminus \nu}. \ \lambda w_s .$ \\ $\exists v_\nu. \ C(v)(w) \wedge \forall w'_s. \ w' \in \mathcal{B}^j_{w} \wedge C(v)(w'). \ \textsc{Pretty}(x)(w')(j)$
		\end{minipage}
	}
\end{center}
...
\begin{exe}
	\ex
	\begin{xlist}
		\ex[] {\textbf{This wedding} was pretty.}\label{ex:nom-event-pc}
		\ex[??] {\textbf{Dancing the waltz} is pretty.}\label{ex:non-nom-event-visual-pc}
		\ex[??] {\textbf{Eating this cherry pie} was delicious.}\label{ex:non-nom-event-gustative-pc}
	\end{xlist}
...
\section*{Conclusion: a typology of predicates with infinitival complements}\label{sec:conclusion}		
	...
	\begin{exe}
		...
		\begin{xlist}
			...
			\textbf{$\leadsto$ Gabby was rude.}}\label{ex:rude-construction-extracted}
			\ex[] {\textbf{It} was rude of Gabby \textbf{to refuse Daiya's invitation}. \hfill \textit{it}-\textit{rude}\\
			\textbf{$\leadsto$ Gabby was rude.}}\label{ex:rude-construction-it}
